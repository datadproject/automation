###############################################################################
# All computation. No resources, no data sources.
###############################################################################

locals {
  # ---------------------------------------------------------------------------
  # Secret reference. ECS accepts an ARN suffix to pluck a key out of a JSON
  # secret: "<arn>:json-key::". Plain-string secrets pass the bare ARN.
  # ---------------------------------------------------------------------------
  ...
  # The module creates the log group unless the caller supplies one.
  log_group_name = coalesce(
    var.sidecar_log_group,
    try(aws_cloudwatch_log_group.sidecars[0].name, null),
  )
  ...
  # Proxy config, only when dd_proxy_https is set. DD_PROXY_NO_PROXY must
  # include 169.254.170.2 or the agent proxies its own ECS task-metadata
  ...
  agent_container = {
    ...
    # Lets the app wait for a genuinely ready agent instead of dropping the
    # first ~30s of startup spans on every deploy.
    healthCheck = {
  ...
  # ---------------------------------------------------------------------------
  # Fragments the caller merges into the app container
  # ---------------------------------------------------------------------------
  app_log_configuration = var.enable_logs ? {
    ...
    options = merge({
      ...
      # fluent-bit's datadog output does not read HTTPS_PROXY from the
      # environment; it needs its own proxy setting.
