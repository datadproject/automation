...
variable "sidecar_log_group" {
  description = <<-EOT
    Existing CloudWatch log group for the sidecars' OWN logs (not app logs).
    Leave null and the module creates one at
    /ecs/<env>/<service>/datadog-sidecars. Set it only when a central team
    owns log-group creation and retention.
  EOT
  type        = string
  default     = null
}

variable "sidecar_log_retention_days" {
  description = "Retention for the module-created log group. Ignored if sidecar_log_group is set."
  type        = number
  default     = 7
}

variable "sidecar_log_kms_key_arn" {
  description = "CMK for the module-created log group. null uses the AWS-managed key."
  type        = string
  default     = null
}
...
variable "execution_role_name" {
  description = <<-EOT
    NAME (not ARN) of the ECS task EXECUTION role. The module attaches an
    inline policy granting read on the Datadog secret.

    The module never creates the role -- the caller owns it. Attaching an
    inline policy to a role you were handed is namespaced per role, so there
    is no collision risk.

    Set to null to opt out and attach execution_role_policy_json yourself.
  EOT
  type        = string
  default     = null
}
...
variable "tags" {
  description = "Tags for module-created resources."
  type        = map(string)
  default     = {}
}
